...
const ItemListDrink = ({ item, getUserInfor }) => {
  ...
  return (
    <LinearGradient colors={['#EEEEEE', '#CCCCCC', '#444444']} style={{ width: 170, height: 290, marginRight: 10, borderRadius: 23 }}>
      ...
      <Text style={{ marginTop: 1, color: 'white', fontSize: 13, marginHorizontal: 15 }}>{item.tooping} </Text>
      <View style={{
        width: '90%',
        flexDirection: 'row',
        ...
        <TouchableOpacity
          onPress={() => {
            ...
            const img = item.img;
            navigation.navigate('CoffeeDetail', { id, name, star, description,img,getUserInfor })
          }}
